...
After compiling the parallel version of MUMPS, which we describe in the [linux](./linux#running-mumps-in-julia)/[windows](./windows#running-mumps-in-julia)/[macOS](./macOS#running-mumps-in-julia). In <code>startup.jl</code>, we should set the Julia environment variable <code>MUMPS_PREFIX</code>, which is the path to your own MUMPS libraries through the terminal: 

```shell
mkdir ~/.julia/config
echo 'ENV["MUMPS_PREFIX"] = "/home/usc/MUMPS_5.6.2/lib"' > ~/.julia/config/startup.jl
```

Please remember to modify <code>"/home/usc/MUMPS_5.6.2/lib"</code> to your own MUMPS libraries path. 
...
After installing MESTI.jl and run <code>[install_packages.jl](../test/install_packages.jl)</code>, we are ready to run the following test scripts
