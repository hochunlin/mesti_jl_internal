function plot_and_compare_distribution(tau)
    # Take the transmission eigenvalue tau (eigenvalue of t^(dag)*t) tau, plot its distribution 
    ...
    plot!(p, bin_centers,mean(tau) ./ (2*bin_centers .* sqrt.(1 .- bin_centers)), label="DMPK theory",  xlims = (0,1), linewidth=3, dpi= 300)
    display(p)
end