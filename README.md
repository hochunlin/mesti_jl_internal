...
MESTI.jl can perform most linear-response computations in 2D and 1D for arbitrary structures, such as
...
Examples would be provided in the [examples](./examples) folder illustrate the usage and the main functionalities of MESTI. Each example has its own folder, with its <code>.jl</code> script, auxiliary files specific to that example, and a <code>README.md</code> page that shows the example script with its outputs:
