...
"""
    MESTI2S Solves frequency-domain scattering problems in a two-sided geometry.
       ...
       syst (Syst struct; required):
          ...
          syst.xBC (string; required unless syst.kx_B is specified):
...
function mesti2s(syst::Syst, input::Union{channel_type, channel_index, wavefront}, output::Union{channel_type, channel_index, wavefront, Nothing}, opts::Union{Opts, Nothing})
    ...
    if ~(isdefined(syst, :epsilon_xx) && isdefined(syst, :epsilon_yy) && isdefined(syst, :epsilon_zz))
        throw(ArgumentError("Input argument syst must have field \"epsilon_xx\", \"epsilon_yy\", and \"epsilon_zz\"."))
    end    
    ...
    elseif isdefined(syst, :PML_type) && (lowercase(syst.PML_type) != "upml")
        throw(ArgumentError("syst.PML_type = \"UPML\" is the only optiona in mesti2s(); use mesti() if SC-PML is neede."))
    end
    ...
    elseif isa(input, channel_index)
        ...
        if ~(isdefined(input, :ind_low_s) || isdefined(input, :ind_low_p) || isdefined(input, :ind_high_s) || isdefined(input, :ind_high_p))
            throw(ArgumentError("When ''input'' is a channel_index strucutre, it must provide field at least one field of \"ind_low_s\", \"ind_low_p\", \"ind_high_s\", and \"ind_high_p\"."))
        end        
        ...
        if ~(isdefined(input, :v_low_s) || isdefined(input, :v_low_p) || isdefined(input, :v_high_s) || isdefined(input, :v_high_p))
    ...
    if output != nothing
        ...
        elseif isa(output, channel_index)
            ...
            if ~(isdefined(output, :ind_low_s) || isdefined(output, :ind_low_p) || isdefined(output, :ind_high_s) || isdefined(output, :ind_high_p))
                throw(ArgumentError("When ''output'' is a channel_index strucutre, it must provide field at least one field of \"ind_low_s\", \"ind_low_p\", \"ind_high_s\", and \"ind_high_p\"."))
            end        
            ...
            if ~(isdefined(output, :v_low_s) || isdefined(output, :v_low_p) || isdefined(output, :v_high_s) || isdefined(output, :v_high_p))
